...
The simplest and most fundamental object in R is the so-called "atomic vector",
commonly referred to simply as "vector". It is a one-dimensional collection of 
elements of the same data type, e.g. numbers, characters, etc.
<br><br>
For example, a numeric vector may consist of elements `12, 14, 20`, and a 
character vector may consist of elements `"x", "y", "z"`.
<br><br>
...
Other data types include environments and closures i.e. functions (@sec-functions).
...
an object of a certain size with some initial values, e.g. all zeros or 
[`NA`s](#na),
in order to fill them in / replace them later.

This is often computationaly faster than growing a smaller objects by appending
...
We can explicitly convert objects of one type to a different type using `as.*` 
functions:
...
```{block, type="rmdnote"}
Note: We had to add `error = TRUE` in the Rmarkdown's code block's options (not 
visible in the HTML output), because otherwise compilation of the Rmarkdown 
document would stop at the error.
```
...
`anyNA()` is a very useful function to check if there an any NA values in an 
object:
...
The chapter on [Handling Missing Data](#missingdata) describes some approaches 
to handling missing data in the context of statistics or machine learning.
...
However, in lists and therefore also data frames, replacing an element with 
`NULL` ***removes that element***:
