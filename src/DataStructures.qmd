...
|----------------:|-------------------:|----------------:|:--------------------------|
| **Vector**      | 1-D                | homogeneous     | the "base" object
| **Matrix**      | 2-D                | homogeneous     | a vector with 2 dimensions
| **Array**       | N-D                | homogeneous     | a vector with N dimensions
| **List**        | 1-D; can be nested | hetereogeneous  | a collection of any R objects, each of any length
| **Data frame**  | 2-D                | hetereogeneous  | a special kind of list: a collection of (column) vectors of any type, all of the same length
...
Data frames are a special kind of list. Their elements are one of more vectors,
which can be of any type and form columns. Therefore a data.frame is a 
two-dimensional data structure where rows typically correspond to cases 
(e.g. individuals) and their columns represent variables. As such, data.frames
...
A vector is the most basic & fundamental data structure in R. 
...
vector, or to combine lists. Otherwise you must inspect the outcome to be 
certain it was as intended.  
...
Unlike a matrix, the elements of a data.frame are the columns, not the 
...
| `data.frame(a = ..., b = ...)` or `data.frame(matrix(NA, x, y))`               | `as.data.frame(x)` | `is.data.frame(x)` |
