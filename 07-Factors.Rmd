...
```{r FigRFactors, echo = FALSE, out.width = "80%", fig.align = 'center', fig.cap = "Factors in R summary - Best to read through this chapter first and then refer back to this figure"}
...
as.numeric(xn)
```
But! We already know this will return the integer index, it will not return the labels as numbers. The way around this is simply to convert to character first and then to numeric:

```{r}
as.numeric(as.character(xn))
```
