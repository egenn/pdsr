...
```{r}
xf <- factor(sample(c("patient_status_positive", "patient_status_negative"), 10, replace = TRUE),
...
```{r}
dat <- sample(c("small", "medium", "large"), 10, replace = TRUE)
x <- factor(dat)
x
```
...
dat <- data.frame(PID = 8001:8009,
                  Sex = factor(sample(c("male", "female"), 9, replace = TRUE)),
                  Group = factor(sample(c("A", "B"), 9, replace = TRUE)))
dat
```
