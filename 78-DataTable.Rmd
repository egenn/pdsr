...
You can similarly convert a `data.table` into a `data.frame`, in-place:
```{r}
setDF(dat)
class(dat)
```
...
Selecting a single column with integer indexing in __data.table__ does not drop to a vector (i.e. similar to `drop = FALSE` in a data.frame):
```{r}
df[, 1]
df[, 1, drop = FALSE]
dt[, 1]
...
In __data.table__, you can access column names directly without quoting or using `$`:
```{r}
df[, "B"]
df$B
dt[, B]
...
dt
```
Same awkward notation to delete multiple columns:
```{r}
...
When you add a new column to an existing, the data.frame is copied behind the scenes (its address in memory changes):
```{r}
...
When you add a new column in a data.table *in-place* its address remains the same:
```{r}
...
Up to now, we have learned about how to use the `data.table` frame `dat[i, j]` to filter cases in `i` or add/remove/transform columns in-place in `j`. There is a whole other dimension in the `data.table` frame: `by`.  
The `data.table` syntax is:
...
For more complex operations, you may need to refer to the slice of the `data.table` defined by `by` within `j`. There is a special notation for that: `.SD` (think sub-`data.table`):
```{r}
