...
List a dataset included with an R package:
```{r, eval = FALSE}
data(package = "glmnet")
data(package = "MASS")
```
...
You can execute any command directly on the underlying OS from within R using the `system` command. (These are, obviously, OS-dependent)
```{r}
system("uname -a")
```
...
If you want to stick to using data frames, you can use the argument `data.table`:
```{r eval = FALSE}
bcw2 <- fread("https://github.com/EpistasisLab/penn-ml-benchmarks/raw/master/datasets/classification/breast-cancer-wisconsin/breast-cancer-wisconsin.tsv.gz",
              data.table = FALSE)
```
...
```{r eval=FALSE}
save.image("workspace_01_29_2020.RData")
```
