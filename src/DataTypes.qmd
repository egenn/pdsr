...
```{block, type="rmdnote"}
You can use either `<-` or `=` for assignment. Many R syntax guides you should only use it 
...
would conflict with the built-in function `data()`.
```
...
[`NA`s](#na), in order to replace with other values later.

This is usually computationaly more efficient than starting with a small object
