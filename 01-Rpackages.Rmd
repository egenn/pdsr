...
x[1:20]
```
Notice that R adds numbers in brackets in the beginning of each row. That number is the integer index of the first element in that row. Therefore the first one is always `[1]` and the number of the subsequent rows depends on how many elements fit in each line. If the output is a single element, it will still have `[1]` in front of it.  
