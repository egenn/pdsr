...
package with the `%>%` symbol. Note that a number of other packages that endorse the use of pipes export the pipe operator as well.  
...
Pipes are used extensively in the [tidyverse](https://www.tidyverse.org) packages and many 
...
- native pipe requires `()` after function name, **magrittr** works with or without
