...
Before starting to use the above functions, we need to learn about anonymous 
functions, which are often used within the apply functions.
...
```{block, type="rmdnote"}
`apply()` only works on objects with defined (i.e. not NULL) `dim()`.
It can be 
```
