...
```{r}
bill_length_by_island <- aggregate(bill_len ~ island,
                                   data = penguins, mean)
bill_length_by_island
```

```{r}
barplot(bill_len ~ island, data = bill_length_by_island)
```

#### Single continuous by multiple categorical variables:

```{r}
bill_length_by_island_x_species <- aggregate(bill_len ~ island + species,
                                             data = penguins, mean)
bill_length_by_island_x_species
```

```{r}
barplot(bill_len ~ island + species,
...
bill_and_flipper_length_by_island <- 
     aggregate(cbind(bill_len, flipper_len) ~ island,
     data = penguins, mean)
bill_and_flipper_length_by_island
```

```{r}
barplot(cbind(bill_len, flipper_len) ~ island, 
...
bill_and_flipper_length_by_island_x_species <- 
     aggregate(cbind(bill_len, flipper_len) ~ island_x_species,
     data = penguins, mean)
bill_and_flipper_length_by_island_x_species
```

```{r}
barplot(cbind(bill_len, flipper_len) ~ island_x_species, 
