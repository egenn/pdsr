...
```{r}
hist(x, col = "slategrey", lwd = 3)
```

```{r}
hist(x, 
     col = adjustcolor("slategray", .5), 
     border = "slategray")
```

Setting border color to the same as the background gives a clean look - though each bar appears slightly shorter than actual.
...
the data to get one value per category. We'll use the penguin data as an 
example.
...
```{r}
barplot(bill_length_mm ~ island, bill_length_by_island)
```
...
```{r}
barplot(bill_length_mm ~ island + species, bill_length_by_island_x_species,
        legend = TRUE)
```
...
<br><br>
Note: here "device" does not refere to a physical device but to  different 
software graphics interfaces that can be used to disaply a plot to screen or 
