...
It's a good idea to keep idea of the version of R and installed packages when beginning a new project. Easy way to do this is to save the output of `sessionInfo()`:
```{r}
sessionInfo()
```
...
### Query the docs
If you do not know the name of what you are looking for, you can use double question marks, `??`, followed by your query, which is short for the `help.search` command, which provides a number of arguments.:
```{r}
??bootstrap
```
