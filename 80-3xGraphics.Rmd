...
Here we use [**`tidyr`**'s](https://tidyr.tidyverse.org/) `pivot_longer()` function, since it is part of the same cult known as the [tidyverse](https://www.tidyverse.org/). You can instead use the builtin [`reshape()`](#reshaping) function or any wide-to-long operation of your choosing, e.g. [**`data.table`**'s](#datatable) [`melt()`](#melt).
...
```{r}
mplot3.x(iris$Petal.Length, "hist", group = iris$Species, hist.breaks = 10)
```

```{r warning=FALSE}
dplot3.x(iris$Sepal.Length, "hist", group = iris$Species)
```
