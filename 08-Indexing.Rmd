...
This is probably quite less common, but potentially useful.
...
mat[idm]
```
### Cartesian product of index vectors

Also rather uncommon: passing two vectors indexes with the cartesian product of the vectors, therefore returning a matrix:

```{r}
mat[c(2, 4, 7), c(4, 3, 1)]
```

To visualize the cartesian product of the two vectors you can use `outer()`:

```{r}
outer(c(2, 4, 7), c(4, 3, 1), paste, sep = ",")
```
