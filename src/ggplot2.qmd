...
palette_a <- adjustcolor(palette_, .666)
```
...
`position = "identity"` displays overlapping bars. Alternatively, `position = "dodge"` can be used to display groups' bars side by side instead. Finally, `position = "stack"`,is the (unforunate) default and results in vertically stacked bars, which can be
confusing.
