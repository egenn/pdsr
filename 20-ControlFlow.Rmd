...
```
Code is often not executed linearly (i.e. line-by-line). Control flow operations define the order in which code segments are executed.
...
Control flow operations form some of the fundamental building blocks of programs. Each operation is very simple - combine enough of them you can bui
...
Instead of using multiple `if` - `else if` statements, we can build a more compact call using `switch`. (It is best suited for options that are of type character, rather tha numeric)
...
## `repeat` loops
`repeat` is not that useful, it initiates an infinite loop and **must** use `break` to exit. Best to use any other type of loop instead.
```{r}
i <- 10
repeat {
 i <- i - 1
