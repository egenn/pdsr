...
Data may be missing for a variety of reasons. We often characterize the type of missingness using the following three types:  
...
Let's load the `PimaIndiansDiabetes2` dataset from package **mlbench** and make a copy of it to variable `dat`:
...
You can count the number of missing values using `sum()`. Remember that a logical array is coerced to an integer array for mathematical operations, where `TRUE` becomes and `FALSE` becomes 0. Therefore calling `sum()` on a logical index counts the number of `TRUE` elements. Since we are applying it on the index of `NA` values, it counts the number of elements with missing values:

```{r}
sum(is.na(dat))
```
There are 652 `NA` values in total.
...
Let's count the number of missing values per case i.r. row:
...
It may be helpful to visualize missing data to get a quick impression of missingness. In **rtemis** you can use `mplot.missing()`. 
...
We lost `r nrow(dat) - nrow(dat_cc)` cases in the above example. That's a lot, so we probably want to look at options were we do not exclude cases.
