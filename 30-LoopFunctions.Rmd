...
```{r FigRapplyfam, echo = FALSE, out.width = "75%", fig.align = 'center', fig.cap = "`*apply()` function family summary (Best to read through this chapter first and then refer back to this figure)"}
...
```{block, type="note"}
`apply()` only works on objects with defined (i.e. non-NULL) `dim()`
```
