...
```{r}
barplot(VADeaths, beside = TRUE, legend.text = TRUE)
```
...
The `**col**` and `**border**` arguments work as expected. Here we define two custom colors using their hexadecimal RGB code and use the solid version for the border and a 50% transparent version for the fill:
