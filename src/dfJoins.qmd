...
where `x` and `y` and the two data.frames to join, and `by` is the column name of the ID variable used to identify rows. If the two datasets' ID column has a different name, e.g. "PatientID" in one and "PID" in the other, you can either rename one of them,
or probably best, use the following syntax:

`merge(x, y, by.x, by.y)`

where `by.x` should be the name of the ID column for the `x` dataset and `by.y` should be the name
of the ID column for the `y` dataset.
