...
`trimws()` removes leading and/or trailing white space from each element of a character vector.

```{r}
x <- c("   Leading space", "Trailing space   ", "  Both sides  ", "NoSpace")
x
```
...
The `which` argument accepts values `"both"` (default), `"left"`, or `"right"` to specify which side(s) to trim:
...
```{r}
x <- c("dem$ID", "dem$Age", "lab$Glucose", "t1w$WBV")
gsub(".*\\$", "", x)
```
