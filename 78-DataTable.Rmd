...
One difference from `data.frame()`, as you can see above, is that `stringsAsFactors` defaults to FALSE in `data.table()`. As part improving efficieny, data.tables do away with row names, which are rarely used. Instead of using rownames, you can and should add an extra column with the same information - this is advisable when working with data.frames as well.
...
## Reshape a `data.table` Wide <=> Long
