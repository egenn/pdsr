...
Once again, just by changing the column names, a new copy of the data.frame was made
behind the scenes and therefore the address has changed.
...
```{r}
setnames(dt1, c("Patient_ID", "GCS", "HR"))
```
...
Once again, an opertion on data.table by reference, does not create a new copy of the
object and therefore the address remains the same.
