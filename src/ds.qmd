...
# Data Queries across data frame implementations {#sec-dataqueryx}
...
- `duckplyr` fails on a simple `select` operation involving a factor and falls back to using regular `dplyr`.
- `tidypolars` fails on a simple `group-by` operation.
:::
