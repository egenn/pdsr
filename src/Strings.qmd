...
`cat()` **does not return any object**. It is therefore useful to produce informative 
messages in your programs.

Use the `file` argument to write to a text file. The `append` argument allows 
using multiple `cat()` calls to append to the same file.

Note: if you use `cat()` on a factor, it will print the integer index of the
factor levels. If you wish to print the levels themselves, use `as.character()`
...
The argument `collapse` helps output a *single* character element after 
collapsing with some string:
...
`nchar()` counts the number of characters in each **element** of type character 
in a vector:
...
character (R base type) vector, by position:
...
`strsplit()` allows you to split a character vector elements based on any 
...
As with any functions, you can compose string operations in complex ways 
(though it may often be considerably easier to perform multiple separate 
operations instead):
...
`abbreviate()` allows to reduce elements of a character vector to short, unique 
abbreviations of a minimumn length (defaults to 4)
...
`grep()`'s `value` arguments which defaults to `FALSE`, allows returning the 
...
### `gsub()`: Find and replace **all** matches of a pattern {#gsub}
...
There are many rules in defining regular expressions and take a little getting 
...
A number of character classes are predefined. Slightly confusingly, they are 
themselves surrounded by brackets and to use them as a character class, you 
need a seconds set of brackets around them. Some of the most common ones 
include:  
