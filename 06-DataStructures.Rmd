...
```{r FigRDataStructures, echo = FALSE, fig.align = 'center', fig.cap = "R Data Structure summary - Best to read through this chapter first and then refer back to this figure"}
...
```{block, type="note"}
A matrix has length (`length(x)`) equal to the number of all (i, j) elements or nrow * ncol
(if `i` is the row index and `j` is the column index) and dimensions (`dim(x)`) as expected:
```
